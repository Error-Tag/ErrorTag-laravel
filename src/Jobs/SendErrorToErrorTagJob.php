...
class SendErrorToErrorTagJob implements ShouldQueue
{
    ...
    protected function reconstructPayload(): ErrorPayload
    {
        ...
        $exception = $this->errorPayload['exception'] ?? [];
        $request = $this->errorPayload['request'] ?? null;
        $user = $this->errorPayload['user'] ?? null;
        $application = $this->errorPayload['application'] ?? [];
        ...
        return new ErrorPayload(
            ...
            request: $request !== null ? new \ErrorTag\ErrorTag\DataTransferObjects\RequestData(
                url: $request['url'] ?? '',
                method: $request['method'] ?? 'GET',
                headers: $request['headers'] ?? [],
                body: $request['body'] ?? null,
                routeName: $request['route_name'] ?? null,
                controller: $request['controller'] ?? null,
                ip: $request['ip'] ?? null,
                userAgent: $request['user_agent'] ?? null,
            ) : null,
            user: $user !== null ? new \ErrorTag\ErrorTag\DataTransferObjects\UserData(
                id: $user['id'] ?? '',
            ...
            application: new \ErrorTag\ErrorTag\DataTransferObjects\ApplicationData(
                laravelVersion: $application['laravel_version'] ?? '',
                phpVersion: $application['php_version'] ?? '',
                environment: $application['environment'] ?? 'production',
                serverName: $application['server_name'] ?? '',
                appName: $application['app_name'] ?? null,
                appUrl: $application['app_url'] ?? null,
                appLocale: $application['app_locale'] ?? null,
            ),
            customContext: $this->errorPayload['custom_context'] ?? [],
            release: $this->errorPayload['release'] ?? null,
            timestamp: $this->errorPayload['timestamp'] ?? now()->toIso8601String(),
        );
    }
}
